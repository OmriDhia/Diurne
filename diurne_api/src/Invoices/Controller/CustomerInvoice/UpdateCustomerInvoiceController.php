...
use App\Invoices\Bus\Command\CustomerInvoice\UpdateCustomerInvoiceCommand;
use App\Invoices\Bus\Command\CustomerInvoice\CustomerInvoiceResponse;
use App\Invoices\DTO\UpdateCustomerInvoiceRequestDto;
...
#[Route('/api/customerInvoices/{id}', name: 'update_customer_invoice', methods: ['PUT'])]
class UpdateCustomerInvoiceController extends CommandQueryController
{
    #[OA\Response(response: 200, description: 'Updated', content: new Model(type: CustomerInvoiceResponse::class))]
    #[OA\RequestBody(
    ...
    public function __invoke(int $id, #[MapRequestPayload] UpdateCustomerInvoiceRequestDto $dto): JsonResponse
    {
        if (!$this->isGranted('update', 'invoice')) {
            return new JsonResponse(['code' => 401, 'message' => 'Unauthorized'], Response::HTTP_UNAUTHORIZED);
        }

        $command = new UpdateCustomerInvoiceCommand(
            id: $id,
        ...
        return SuccessResponse::create('customer_invoice_updated', $response->toArray());
    }
}
