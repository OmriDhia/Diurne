...
use App\Invoices\Bus\Command\SupplierInvoice\UpdateSupplierInvoiceCommand;
use App\Invoices\Bus\Command\SupplierInvoice\SupplierInvoiceResponse;
use App\Invoices\DTO\UpdateSupplierInvoiceRequestDto;
...
#[Route('/api/supplierInvoices/{id}', name: 'update_supplier_invoice', methods: ['PUT'])]
class UpdateSupplierInvoiceController extends CommandQueryController
{
    #[OA\Response(response: 200, description: 'Updated', content: new Model(type: SupplierInvoiceResponse::class))]
    #[OA\RequestBody(
    ...
    public function __invoke(int $id, #[MapRequestPayload] UpdateSupplierInvoiceRequestDto $dto): JsonResponse
    {
        if (!$this->isGranted('update', 'invoice')) {
            return new JsonResponse(['code' => 401, 'message' => 'Unauthorized'], Response::HTTP_UNAUTHORIZED);
        }

        $command = new UpdateSupplierInvoiceCommand(
            id: $id,
        ...
        return SuccessResponse::create('supplier_invoice_updated', $response->toArray());
    }
}
