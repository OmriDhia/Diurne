  ...
  <script setup>
    ...
    const fetchData = async ({ page, itemsPerPage } = {}) => {
      try {
        ...
        const { data } = await axiosInstance.get('/api/specialTreatment', { params });
    ...
    const addData = async (row) => {
      try {
        ...
        const { data } = await axiosInstance.post('/api/specialTreatment/create', row);
    ...
    const saveData = async (row) => {
      try {
        row.price = parseFloat(row.price);
        const { data } = await axiosInstance.put(`/api/specialTreatment/${row.id}`, row);
