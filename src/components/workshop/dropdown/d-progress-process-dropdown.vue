<template>
    <div class="row align-items-center">
        ...
        <div class="col-12">
            <select id="processDropdown" :disabled="disabled" :class="{ 'is-invalid': error, 'form-select': true }" :value="discount" @change="handleChange($event.target.value)">
                <option v-for="(prof, key) in discounts" :key="key" :value="prof.id">{{ prof.name }}</option>
            </select>
        </div>
        <div v-if="error" class="invalid-feedback">{{ $t('Processus est obligatoire.') }}</div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async getDiscounts() {
            try {
                const res = await axiosInstance.get('/api/progressReportProcess');
