<template>
    <div>
        <d-base-modal id="downloadWorkshopFacture" title="Téléchargement bon de commande atelier" @onClose="handleClose">
...
<script setup>
    ...
    onMounted(() => {
        if (props.workshopOrderId) {
            getWorkshopHtml(props.workshopOrderId);
        }
    });
    
    const getWorkshopHtml = async (id) => {
        try {
            if (id) {
                const fetchedHtml = await workshopService.getWorkshopHtml(id);
