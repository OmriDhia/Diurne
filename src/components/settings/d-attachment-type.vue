<template>
    <d-data-grid :fetchData="fetchData" :saveData="saveData" :addData="addData" :deleteData="deleteData"
      :columns="columns" :rows="rows" title="Type de pièce jointe" rowKey="id" />
  </template>
  ...
  <script setup>
    ...
    const fetchData = async ({ page, itemsPerPage }) => {
      try {
        const { data } = await axiosInstance.get('/api/attachment-types', {
          params: {
            page,
            itemsPerPage
          }
    ...
    const addData = async (row) => {
      try {
        const { data } = await axiosInstance.post('/api/attachment-type/create', row);
    ...
    const deleteData = async (row) => {
      try {
        await axiosInstance.delete(`/api/attachment-type/delete/${row.id}`);
    ...
    const saveData = async (row) => {
      try {
        const { data } = await axiosInstance.put(`/api/attachment-type/update/${row.id}`, row);
