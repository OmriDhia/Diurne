<template>
  <d-data-grid :fetchData="fetchData" :saveData="saveData" :addData="addData" :deleteData="deleteData"
    :columns="columns" :rows="rows" title="Transporteur" rowKey="id" />
</template>
...
<script setup>
  ...
  const fetchData = async ({ page, itemsPerPage }) => {
    try {
      const { data } = await axiosInstance.get('/api/carrier', {
        params: {
          page,
          itemsPerPage
        }
  ...
  const addData = async (row) => {
    try {
      const { data } = await axiosInstance.post('/api/createCarrier', row);
  ...
  const deleteData = async (row) => {
    try {
      await axiosInstance.delete(`/api/carrier/${row.id}`);
  ...
  const saveData = async (row) => {
    try {
      const { data } = await axiosInstance.put(`/api/updateCarrier/${row.id}`, row);
