<template>
    <d-data-grid :fetchData="fetchData" :saveData="saveData" :addData="addData" :deleteData="deleteData"
      :columns="columns" :rows="rows" title="Types de transport" rowKey="id" />
  </template>
  ...
  <script setup>
    ...
    const fetchData = async ({ page, itemsPerPage }) => {
      try {
        const { data } = await axiosInstance.get('/api/transportType', {
          params: {
            page,
            itemsPerPage
          }
    ...
    const addData = async (row) => {
      try {
        const { data } = await axiosInstance.post('/api/transportType/create', row);
    ...
    const deleteData = async (row) => {
      try {
        await axiosInstance.delete(`/api/transportType/${row.id}`);
    ...
    const saveData = async (row) => {
      try {
        const { data } = await axiosInstance.put(`/api/updateTransportType/${row.id}`, row);
