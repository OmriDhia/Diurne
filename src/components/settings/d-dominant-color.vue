<template>
    <d-data-grid :fetchData="fetchData" :saveData="saveData" :addData="addData" :deleteData="deleteData"
      :columns="columns" :rows="rows" title="Couleurs dominantes" rowKey="id" />
  </template>
  ...
  <script setup>
    ...
    const columns = [
      ...
      { key: "hexCode", label: "hXexCode", type: "text" },
    ];
    ...
    const fetchData = async ({ page, itemsPerPage }) => {
      try {
        const { data } = await axiosInstance.get('/api/dominant-colors', {
          params: {
            page,
            itemsPerPage
          }
    ...
    const addData = async (row) => {
      try {
        const { data } = await axiosInstance.post('/api/dominant-color/create', row);
    ...
    const deleteData = async (row) => {
      console.log(row);
      try {
        await axiosInstance.delete(`/api/dominant-color/${row.id}`);
    ...
    const saveData = async (row) => {
      try {
        const { data } = await axiosInstance.put(`/api/dominant-color/update/${row.id}`, row);
