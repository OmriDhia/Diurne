<template>
  <d-data-grid :fetchData="fetchData" :saveData="saveData" :addData="addData" :deleteData="deleteData"
    :columns="columns" :rows="rows" title="Devise" rowKey="id" />
</template>
...
<script setup>
  ...
  const fetchData = async ({ page, itemsPerPage } = {}) => {
    try {
      ...
      const { data } = await axiosInstance.get('/api/currency', { params });
  ...
  const addData = async (row) => {
    try {
      const { data } = await axiosInstance.post('/api/createCurrency', row);
  ...
  const deleteData = async (row) => {
    console.log(row);
    try {
      await axiosInstance.delete(`/api/currency/${row.id}`);
  ...
  const saveData = async (row) => {
    try {
      const { data } = await axiosInstance.put(`/api/updateCurrency/${row.id}`, row);
