<template>
    <d-data-grid :fetchData="fetchData" :saveData="saveData" :addData="addData" :deleteData="deleteData"
      :columns="columns" :rows="rows" title="Coleurs" rowKey="id" />
  </template>
  ...
  <script setup>
    ...
    const addData = async (row) => {
      try {
        const { data } = await axiosInstance.post('/api/color/create', row);
    ...
    const deleteData = async (row) => {
      console.log(row);
      try {
        await axiosInstance.delete(`/api/color/${row.id}`);
    ...
    const saveData = async (row) => {
      try {
        const { data } = await axiosInstance.put(`/api/color/${row.id}`, row);
